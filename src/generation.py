...
class SecurityLogitsProcessor(LogitsProcessor):
    ...
    def __call__(
        ...
        raw_lm_probs = F.softmax(scores, dim=1) if if_scores_raw_logits else torch.exp(scores)        
        if self.top_k > 0:
        ...
        secure_probs = 1 + security_scores - self.threshold
...
class CustomizedGenerationMixin:
    ...
    def __init__(self, model, config, security=None) -> None:
        self.model = model
    ...
    def adjust_logits_during_generation(self, logits: torch.FloatTensor, **kwargs) -> torch.FloatTensor:
        ...
        """
        # return logits
        return self.model.adjust_logits_during_generation(logits, **kwargs)
    ...
    def generate(
        ...
        **model_kwargs,
        ...
        if self.config.is_encoder_decoder and "encoder_outputs" not in model_kwargs:
            ...
            if self.security:
                # get security encoder outputs
                security_kwargs = deepcopy(model_kwargs)
                security_kwargs.pop("encoder_outputs")
                security_kwargs = self._prepare_encoder_decoder_kwargs_for_generation(
                    self.security, inputs_tensor, security_kwargs, model_input_name,
                )
        ...
        # print("- processors: {}".format(logits_processor))
        ...
        if is_greedy_gen_mode:
            ...
            if self.security:
                _, security_kwargs = self._expand_inputs_for_generation(
                    input_ids_keep, expand_size=num_beams, is_encoder_decoder=self.config.is_encoder_decoder, **security_kwargs
                )
                # model_kwargs["encoder_outputs"] = encoder_outputs
                model_kwargs["security_encoder_outputs"] = security_kwargs["encoder_outputs"]
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # =========================== VISIUALIZATION ===============================
            if self.security is not None and next_tokens.shape[0] == 1:

                if visualization_tokenizer is None:
                    from transformers import AutoTokenizer
                    # visualization_tokenizer = AutoTokenizer.from_pretrained("facebook/bart-large")
                    visualization_tokenizer = AutoTokenizer.from_pretrained("gpt2")

                TOP_K = 10
                lm_probs = nn.functional.softmax(next_token_logits, dim=-1)[0]
                topk_probs, topk_ids = torch.topk(lm_probs, TOP_K)

                secured_probs = next_tokens_scores[0]
                secured_topk_probs, secured_topk_ids = torch.topk(secured_probs, TOP_K)
                
                # print(torch.sum(torch.topk(lm_probs, 30)[0]).item())

                threshold = logits_warper[0].threshold
                print('Top-5:\t', [visualization_tokenizer.decode(torch.tensor([i])) for i in topk_ids.tolist()])
                print('Prob:\t', [round(p, 4) for p in topk_probs.tolist()])
                print('1+Q:\t', [round((Q_values + 1)[0][i].item() - threshold, 4) for i in topk_ids.tolist()])
                print('Prob_:\t', [round(p, 4) for p in secured_probs[topk_ids].tolist()])
                print('After:\t', [visualization_tokenizer.decode(torch.tensor([i])) for i in secured_topk_ids.tolist()])

                cur_token_str = visualization_tokenizer.decode(next_tokens)
                # cur_lm_prob = nn.functional.softmax(next_token_logits, dim=-1)[0][next_tokens[0]]
                # cur_secure_prob = (Q_values + 1)[0][next_tokens[0]]
                print('Selected:\t', cur_token_str)
                print()

                # print('- {:<15} LM: {:.4} \t 1 + Q(S, A): {:.3}'.format(cur_token_str, cur_lm_prob, cur_secure_prob))
            # ===========================================================================
